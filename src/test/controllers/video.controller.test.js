...
// define bộ test case cho function getVideos
/**
...
describe("getVideos", () => {
  ...
  beforeEach(() => {
    ...
    // res.status().json() => dùng sinon để giả lập res
    res = {
      status: Sinon.stub().returnsThis(),
      json: Sinon.stub(),
    };

    // giả lập function findAll của ORM => dùng sinon
    // giả lập cho model.video.findAll()
  ...
  it("getVideos fail", async () => {
    ...
    // expect kết quả - so sánh kết quả giữa giả lập và function thật
    expect(res.status.calledOnceWith(500)).to.be.true;

    expect(res.json.calledOnceWith({ message: "error for API get list videos 123" })).to.be.true;
  });
});
